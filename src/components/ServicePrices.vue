  ...
  <q-list bordered class="rounded-borders" style="max-width: 350px">
    <template v-for="p in servicePrices" :key="p.priceName">
      <q-item
        clickable
        v-ripple
        @click="$emit('on-select', { serviceName: serviceName, price: p })"
      >
        <q-popup-proxy v-if="editable" context-menu>
          <div class="row q-gutter-sm bg-blue-grey-14 text-white">
            ...
            <q-btn flat dense icon="las la-trash" @click="onDelete(p)"
              ><q-tooltip>{{ $t("deleteCategoryOrService") }}</q-tooltip></q-btn
            >
          </div>
        </q-popup-proxy>
        <q-item-section>
        ...
        <q-item-section side top>
          <q-item-label
            ...
            }}<q-icon name="las la-euro-sign" color="orange-14"
          /></q-item-label>
          {{ p.unitName }}
        </q-item-section>
      </q-item>
...
const { prices, saveData } = useInvoiceStore();
...
const onNew = (p) => {
  prices.value.push({ ...p, serviceName: props.serviceName });
};
const onEdit = (p) => {
...
const servicePrices = computed(() =>
  thePrices.filter((f) => f.serviceName === props.serviceName)
);
</script>
