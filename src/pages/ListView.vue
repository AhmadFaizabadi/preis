<template>
  <q-table ref="tableRef" dense :rows="rows" :columns="columns" :row-key="rowKey" v-model:pagination="pagination"
    ...
    <template v-slot:body-cell="props">
      <slot name="body-cell" v-bind="props"><q-td :props="props">
          {{ props.value }}
        </q-td></slot>
...
async function onRequest(req) {
  ...
  try {
    if (req.filter?.length > 0) {
      ...
      const wc = 'where ' + segs.map(m => `${props.filterOnField} ILike '%${m}%'`).join(' and ')
      req.pagination.whereClause = wc
    } else req.pagination.whereClause = ""
    const rc = await getAction(props.apiUrlCount, { conditions: req.pagination.whereClause });
