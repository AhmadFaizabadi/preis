...
const onDelete = () => {
  $q.dialog({
    title: t('confirm'),
    message: t(`would you like to ${undelete.value ? 'enable' : 'disable'} this customer?`),
    cancel: true,
    persistent: true
  }).onOk(async () => {
    const res = await deleteAction(`api/v1/customers/${undelete.value ? 'undelete/' : ''}${model.value.Id}`)
