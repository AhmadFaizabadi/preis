<template>
  <q-page padding>
    <q-card class="my-card bg-grey-12 q-pa-md">
      <q-card-section>
        <div class="text-h6">{{ isNew ? $t('newInvoice') : $t('editInvoice') }}</div>
      </q-card-section>
      <q-card-section><q-input filled v-model="model.date" mask="date" :rules="['date']">
        ...
        <q-list>
          <template v-for="item in model.items" :key="item.service.fullName">
            <q-item clickable v-ripple>
              <q-item-section>
                <q-item-label overline>{{ item.service.fullName.split('-')[0] }}</q-item-label>
                <q-item-label>{{ item.service.label }}</q-item-label>
                <q-item-label caption>{{ item.count }} x {{ item.service.unitName }}</q-item-label>
              </q-item-section>
      ...
      <q-card-actions class="justify-end">
        <q-btn v-close-popup @click="$emit('update:model-value', model)" :label="$t('save')" />
...
const props = withDefaults(defineProps({ modelValue: Object }),
  {
    modelValue: {
      date: new Date, customer: null, items: []
    }
  })
</script>
